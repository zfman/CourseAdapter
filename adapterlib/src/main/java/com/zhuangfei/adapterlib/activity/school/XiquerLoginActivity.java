...
import com.zhuangfei.adapterlib.utils.TimetableTools;
...
public class XiquerLoginActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String TAG = "XiquerLoginActivity";
    Button loginButton;
    ...
    private void initView() {
        Object obj=getIntent().getSerializableExtra("selectSchool");
        ...
        findViewById(R.id.user_select_school_layout).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent=new Intent(XiquerLoginActivity.this,ChooseSchoolActivity.class);
                startActivityForResult(intent,REQUEST_CODE_XIQUER);
            }
        });
    ...
    public void onActivityReenter(int resultCode, Intent data) {
        ...
        if(data!=null){
            Object object=data.getSerializableExtra("school");
            if(object!=null&&object instanceof GreenFruitSchool){
                selectSchool= (GreenFruitSchool) object;
                if(!TextUtils.isEmpty(selectSchool.getXxmc())){
                    userSchool.setText(selectSchool.getXxmc());
                }
            }
        }
                    ...
                    public void onResponse(Call<GreenFruitCourse> call, Response<GreenFruitCourse> response) {
                        ...
                        if(fruitCourse!=null){
                            if(fruitCourse.getWeek1()==null&&fruitCourse.getWeek2()==null&&
                                ...
                                if(week==-1){
                                    curWeek=Integer.parseInt(fruitCourse.getZc());
                                    ParseManager.setCurweek(curWeek);
                                }
    ...
    private void parseResult2(GreenFruitCourse fruitCourse) {
        ...
        parseResult(results,weekList1,1);
        parseResult(results,weekList2,2);
        parseResult(results,weekList3,3);
        parseResult(results,weekList4,4);
        parseResult(results,weekList5,5);
        parseResult(results,weekList6,6);
        parseResult(results,weekList7,7);

        ParseManager.setData(results);
        ParseManager.setSuccess(true);
        finish();
    }


    private void parseResult(List<ParseResult> results,List<GreenFruitCourse.WeekBean> weekList1,int day) {
        ...
        try{
            for(GreenFruitCourse.WeekBean weekBean:weekList1){
                ...
                if(json==null||!courseMap.containsKey(json)){
                    ...
                    model.setStep(step);
                    model.setWeekList(TimetableTools.getWeekList(weekBean.getSkzs(),weekBean.getDsz()));
                    results.add(model);
